...
const TreeView = ({
  ...
  const flattenedFlow = useMemo(() => {
    return getFlattenedFlow(nodes, rootNodes);
  }, [nodes, rootNodes]);
  ...
  useEffect(() => {
    ...
    if (layoutMode === LAYOUT_MODE.TREE) {
      ...
      for (let i = 0; i < leafNodes.length; i++) {
        ...
        const pos =
          flowOrientation === FLOW_ORIENTATION.HORIZONTAL ? { x: currentOffset, y: 0 } : { x: 0, y: currentOffset };

        displayNodes.push({ data: node, pos });

        // Advance offset for next node
        const step =
          flowOrientation === FLOW_ORIENTATION.HORIZONTAL
      ...
      allParents.forEach((parentNode) => {
        ...
        if (groupLeaves.length > 0) {
          ...
          enclosures.push({
            ...
            width:
              flowOrientation === FLOW_ORIENTATION.HORIZONTAL
                ? maxX - minX + NODE_WIDTH + hPadding * 2
                : NODE_WIDTH + hPadding * 2,
            height:
              flowOrientation === FLOW_ORIENTATION.HORIZONTAL
                ? NODE_HEIGHT + (vPaddingTop + vPaddingBottom)
                : maxY - minY + NODE_HEIGHT + (vPaddingTop + vPaddingBottom),
    ...
    if (layoutMode === LAYOUT_MODE.FLOW) {
      const enclosureGroups = g
        .selectAll(SELECTOR.ENCLOSURE_GROUP)
        .data(enclosures)
        .enter()
        .append('g')
    ...
    // Links
    const allLinks = g
      .selectAll(SELECTOR.LINK_PATH)
      .data(displayLinks)
      .enter()
      .append('path')
      .attr(
        'class',
        (d) => `${CLASS_NAME.LINK_PATH} ${d.type === LINK_TYPE.FLOW ? CLASS_NAME.FLOW_LINK : CLASS_NAME.TREE_LINK}`
      )
      .attr('d', (d) => {
        if (layoutMode === LAYOUT_MODE.TREE) {
          return d3
    ...
    const nodeGroups = g
      ...
      .on('mouseenter', (event, d) => {
        allLinks
          .filter((l) => l.source.data.id === d.data.id || l.target.data.id === d.data.id)
    ...
    const titleContainer = nodeGroups
    ...
    titleContainer.each(function (d) {
      ...
      if (isEditing) {
        const input = container
          .append(SVG_NAMESPACE_TAG.INPUT)
    ...
    if ((currentTransform.k === 1 && currentTransform.x === 0 && currentTransform.y === 0) || layoutChanged) {
      const initialTransform = d3.zoomIdentity
        .translate(
          flowOrientation === FLOW_ORIENTATION.VERTICAL && layoutMode === LAYOUT_MODE.FLOW
            ? width / 2 - FLOW_VERTICAL_INITIAL_X_OFFSET
            : width / INITIAL_X_DIVISOR,
          height / INITIAL_Y_DIVISOR
        )
        .scale(INITIAL_ZOOM);
    ...
    <div className="tree-view-container" ref={containerRef}>
      <div className="tree-controls">
        <div className="control-group-glass">
          <button
            className={`mode-btn ${layoutMode === LAYOUT_MODE.TREE ? 'active' : ''}`}
            onClick={() => setLayoutMode(LAYOUT_MODE.TREE)}
          >
            <Share2 size={14} /> {t('tree.modeTree')}
          </button>
          <button
            className={`mode-btn ${layoutMode === LAYOUT_MODE.FLOW ? 'active' : ''}`}
            onClick={() => setLayoutMode(LAYOUT_MODE.FLOW)}
          >
        ...
        <div className={`orientation-controls-wrapper ${layoutMode === LAYOUT_MODE.FLOW ? 'is-visible' : ''}`}>
          <div className="control-group-glass">
            <button
              className={`mode-btn ${flowOrientation === FLOW_ORIENTATION.HORIZONTAL ? 'active' : ''}`}
              onClick={() => setFlowOrientation(FLOW_ORIENTATION.HORIZONTAL)}
            >
              <MoveRight size={14} /> {t('tree.orientationHorizontal')}
            </button>
            <button
              className={`mode-btn ${flowOrientation === FLOW_ORIENTATION.VERTICAL ? 'active' : ''}`}
              onClick={() => setFlowOrientation(FLOW_ORIENTATION.VERTICAL)}
            >
        ...
        {showSettings && (
          <div className="control-group-glass tree-settings-floating-wrapper">
            <div className="tree-panel-header">
              ...
              <button className="tree-close-panel-btn" onClick={() => setShowSettings(false)}>
                <X size={14} />
              </button>
            </div>
            <div className="tree-settings-content">
              <div className="tree-setting-item">
                ...
                <input
                  ...
                  step={SPACING_V.step}
                  value={spacingV}
                  onChange={(e) => setSpacingV(parseInt(e.target.value))}
                />
              </div>
              <div className="tree-setting-item">
                <label>
                  {t('tree.spacingH')}: {spacingH}
                </label>
                <input
                  type="range"
                  min={SPACING_H.min}
                  ...
                  step={SPACING_H.step}
                  value={spacingH}
                  onChange={(e) => setSpacingH(parseInt(e.target.value))}
                />
              </div>
              <div className="tree-setting-item">
                <label>
                  {t('tree.containerWidth')}: {containerHPadding}
                </label>
                <input
                  type="range"
                  min={CONTAINER_H_PADDING.min}
                  max={CONTAINER_H_PADDING.max}
                  step={CONTAINER_H_PADDING.step}
                  value={containerHPadding}
                  onChange={(e) => setContainerHPadding(parseInt(e.target.value))}
                />
              </div>
              <div className="tree-setting-item">
                <label>
                  {t('tree.labelGapV')}: {containerVPaddingTop}
                </label>
                <input
                  type="range"
                  min={CONTAINER_V_PADDING_TOP.min}
                  max={CONTAINER_V_PADDING_TOP.max}
                  step={CONTAINER_V_PADDING_TOP.step}
                  value={containerVPaddingTop}
                  onChange={(e) => setContainerVPaddingTop(parseInt(e.target.value))}
                />
              </div>
              <div className="tree-setting-item">
                <label>
                  {t('tree.hierarchyGap')}: {hierarchyGap}
                </label>
                <input
                  type="range"
                  min={HIERARCHY_GAP.min}
                  max={HIERARCHY_GAP.max}
                  step={HIERARCHY_GAP.step}
                  value={hierarchyGap}
                  onChange={(e) => setHierarchyGap(parseInt(e.target.value))}
                />
              </div>
            </div>
          </div>
